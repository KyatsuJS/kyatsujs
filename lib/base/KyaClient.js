...
class KyaClient {
    ...
    constructor(options) {
        ...
        this.Voice = new modules_1.VoiceManager(this);
    ...
    async login(token) {
        ...
        if (this.Voice.events.size > 0) {
            ...
            if (!event)
                event = new index_1.Event(this, 'voiceStateUpdate');
            this.resolved.on('voiceStateUpdate', async (oldState, newState) => {
                const changes = modules_1.VoiceManager.getChanges(oldState, newState);
                const user = await (0, tools_1.SFToUser)(this.resolved, newState.id);
                const context = new services_1.Context(newState.channel || oldState.channel, null, null, user);
                context.client = this;
