...
class Voice {
    ...
    get channel() {
        ...
        if (!this._guild) {
            ...
            this._guild = this.client.guilds.cache.first().id;
        }
        let resolved = this.guild.channels.resolve(this._channel);
        if (!resolved)
            resolved = this.guild.channels.cache.find((c) => c.name.toLowerCase() === this._channel.toLowerCase());
        return resolved;
    }
    /**
    ...
    async disconnect(member, channel) {
        const memberInstance = (await this.contextualize(member, channel || this._channel))[0];
    ...
    async contextualize(member, channel) {
        ...
        if (typeof member === 'string')
            member = await (0, tools_1.SFToMember)(this.client, this._guild, member);
